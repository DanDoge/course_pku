...
ams_sketch::ams_sketch(int c, int c_w, int* s): hash_table(32, c_w, c), seeds(s, s + c){
    cells = c;
    cell_width = c_w;
}
...
void ams_sketch::insert_element(const char* ele, int freq){
	std::cout << "!" << std::endl;
    for(int i = 0; i < cells; i += 1){
        char hash_val[16] = {};
        char mult_fac[16] = {};
        MurmurHash3_x64_128(ele, strlen(ele), seeds[i], hash_val);
        MurmurHash3_x64_128(ele, strlen(ele), seeds[i], mult_fac);
        update_sketch(*(int* )mult_fac, i, *(int* )hash_val, freq);
    }
}

int ams_sketch::update_sketch(int mult, int idx, int hash, int freq){
	std::cout << "in update" << mult << " " << idx << " " << hash << " " << freq << std::endl;
    if (mult & 1) {
...
int main(){
	int seeds[4] = {2, 3, 5, 7};
	ams_sketch as(4, 4, seeds);
	char s[] = "sss";
    as.insert_element(s, 1);

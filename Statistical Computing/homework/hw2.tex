...
\begin{homeworkProblem}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.5\textwidth]{SGHMC.png}
        \caption{KL of different SGHMC methods with batch\_size=32}
        \label{4}
    \end{figure}
    We can conclude that
    \begin{itemize}
        \item as for performance, SGLD $<$ SGHMC $<$ SGNHT
        \item SGLD seems to be stuck somewhere, while SGHMC and SGNHT are able to traverse the posterior(and thus the unstability)
        \item for convergence rate and average KL after convergence, SGLD $<$ SGHMC $<$ SGNHT
    \end{itemize}
\end{homeworkProblem}
